...
def tile_raster_images(X,
    ...
    and also columns of matrices for transforming those rows (such as the
    first layer of a neural net).

    :type X: a 2D ndarray or a tuple of 4 channels, elements of which can
             be 2D ndarrays or None
    :param X: a 2D array in which every row is flattened image
    ...
    :rtype: a 2D array with same dtype as X.
    """
    ...
    out_shape = [
        (ishp + tsp) * tshp - tsp for ishp, tshp, tsp in zip(img_shape, tile_shape, tile_spacing)
    ]
    ...
    if isinstance(X, tuple):
        ...
        for i in range(4):
            if X[i] is None:
                ...
                # in the output
                out_array[:, :, i] = tile_raster_images(X[i],
                                                        img_shape,
                                                        tile_shape,
                                                        tile_spacing,
        ...
        for tile_row in range(tile_shape[0]):
            for tile_col in range(tile_shape[1]):
                if tile_row * tile_shape[1] + tile_col < X.shape[0]:
                    ...
                    # add the slice to the corresponding position in 
                    # the output array
                    c = 1
                    if output_pixel_vals:
                        c = 255
